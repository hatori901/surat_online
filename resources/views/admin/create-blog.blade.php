...
<section class="py-12 max-w-md mx-auto relative z-10 px-4 sm:max-w-3xl sm:px-6 lg:max-w-7xl lg:px-8">
    <div class="bg-gray-200 shadow-md rounded-md">
        <h2 class="text-2xl font-bold mx-5 py-5">Buat Artikel</h2>
        <form method="POST" action="{{ route('blogs.store') }}" class="py-5 px-4">
            @csrf
            @method('POST')
            <div>
                <label for="title" class="block text-lg px-3 font-medium text-gray-700">Judul Artikel</label>
                <div class="mt-1">
                    <input type="text" name="title" id="title" class="shadow-sm text-md py-3 px-4 focus:ring-indigo-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Judul Artikel">
                </div>
            </div>
            <div class="my-5">
                <label for="title" class="block text-lg px-3 font-medium text-gray-700">Artikel</label>
                <textarea name="article" id="artikel" cols="50" rows="50"></textarea>
            </div>
            <div>
